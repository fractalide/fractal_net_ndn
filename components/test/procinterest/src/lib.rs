...
component! {
  net_ndn_test_procinterest, contracts(interest, data, generic_text)
  inputs(),
  ...
  fn run(&mut self) -> Result<()> {
    ...
    let inbound_interest = {
        let inbound_interest_reader: interest::Reader = ip_inbound_interest.read_contract()?;
        inbound_interest_reader.get_name() // read contract: interest to replace XXX
    };
    let mut out_ip_outbound_data = IP::new();
    {
      let mut variable = out_ip_outbound_data.build_contract::<data::Builder>();
